...
/*if(isset($_POST['login_button'])) {
	...
	if($check_login_query == 1) {
		...
		$user_block_query = mysqli_query($con, "SELECT * FROM users WHERE email='$email' AND block='yes'");
		//для приватного аккаунта
...
if(isset($_POST['login_button'])) {
	...
	$check_database_query = mysqli_query($con, "SELECT * FROM users WHERE email='$email' AND password='$password' and block='no'");
	$check_login_query = mysqli_num_rows($check_database_query);
	
	$check_database_query1 = mysqli_query($con, "SELECT * FROM users WHERE block='yes' AND email='$email' ");
...
if(isset($_POST['login_button'])) {
	...
	//$email = ($_POST['log_email']); //херота 
	$_SESSION['log_email'] = $email; //Store email into session variable 
	$password = ($_POST['log_password']); //Get password

	$check_database_query = mysqli_query($con, "SELECT * FROM admin_info WHERE admin_email='$email' AND password='$password'");
	$check_login_query = mysqli_num_rows($check_database_query);
	//echo "<script>window.location='./admin/page.php?page=dashboard'</script>";
	if($check_login_query == 1) {
		$row = mysqli_fetch_array($check_database_query);
		...
		$_SESSION['username'] = $username;
		//echo "<script>window.location='./admin/page.php?page=dashboard'</script>";
		header("Location: http://localhost/IUSocial-master/admin/page.php?page=dashboard");
		//echo '<URL=http://localhost/IUSocial-master/messages.php">'; exit();
...
if(isset($_POST['login_button'])) {
	...
	$check_database_query1 = mysqli_query($con, "SELECT * FROM users WHERE email='$email' AND password='$password'");
	$check_login_query1 = mysqli_num_rows($check_database_query1);
	if($check_login_query1 == 1) {
		$row = mysqli_fetch_array($check_database_query1);
		$username = $row['username'];
