...
class Notification {
	...
	public function getNotifications($data, $limit) {
		...
		if(mysqli_num_rows($query) == 0) {
			...
			echo "<p style='text-align:center'>У вас нет уведомлений!</p>";
			return;
		}
		...
		while($row = mysqli_fetch_array($query)) {
			...
			if($interval->y >= 1) {
				...
				if($interval->h == 1) {
					...
					$time_message = $interval->h . " часa назад";
				}
			}
		...
		if($count > $limit)
			...
			$return_string .= "<input type='hidden' class='noMoreDropdownData' value='true'> <p style='text-align: center;'>Больше нет уведомлений для загрузки!</p>";
	...
	public function insertNotification($post_id, $user_to, $type) {
		...
		switch($type) {
			case 'comment':
				$message = $userLoggedInName . " прокомментировал ваш пост";
				break;
			case 'like':
				$message = $userLoggedInName . " понравился пост";
				break;
			case 'profile_post':
				$message = $userLoggedInName . " опубликовано в вашем профиле";
				break;
			case 'comment_non_owner':
				$message = $userLoggedInName . " прокомментировал сообщение, которое вы прокомментировали";
				break;
			case 'profile_comment':
				$message = $userLoggedInName . " прокомментировал свой пост в профиле";
				break;
		}
